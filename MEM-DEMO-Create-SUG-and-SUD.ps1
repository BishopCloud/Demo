...
$($ENV:SMS_ADMIN_UI_PATH).displayroot



Get-command new-cm*package*
...
If($Updates.count -lt 1000)
{
    ...
    $Answer = Read-Host -Prompt "Do you want to create SUG using the updates listed above? ( Y / N)"
     ...
     $AnswerDP = Read-Host -Prompt "Do you want to create a new deployment package with the SUG just created? ( Y / N)"
         if($AnswerDP -eq "Y")
            {
                # This line computes the UNC path to a folder for the update deployment package but does not actually create the directory.  i.e. \\FMM31597MEM001\E$\2021-JANUARY-NEEDED
                $DeploymentPackagePath = $("\\" + $env:Computername + "\" + (Get-Item $ENV:SMS_ADMIN_UI_PATH).PSDrive.Name + "$\" + $NewSUGName)  
                #This line creates the new software update deployment package and uses the UNC path calculated above as the source directory.
                $DP = New-CMSoftwareUpdateDeploymentPackage -Name $NewSUGName  -Fast -Path $DeploymentPackagePath
                Write-Host $("Created Software Update Deployment Package: " + $DP.Name)

            }
         else
            {
                Write-Host "Exiting without creating SUD"
            }
        }
     else
        {
